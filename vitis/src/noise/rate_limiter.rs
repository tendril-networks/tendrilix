...
use crate::noise::{
    HandshakeInit, HandshakeResponse, Packet, TunnResult, WireGuardError,
    handshake::{LABEL_COOKIE, LABEL_MAC1},
    parse_incoming_packet,
};

#[cfg(feature = "std")]
type RateLimiterMutex<T> = Mutex<T>;
#[cfg(not(feature = "std"))]
type RateLimiterMutex<T> = Mutex<CriticalSectionRawMutex, T>;

use subtle::ConstantTimeEq;

const COOKIE_REFRESH: u64 = 120;
...
pub struct RateLimiter {
    ...
    last_reset: RateLimiterMutex<Instant>,
}

impl RateLimiter {
    pub fn new<R>(public_key: &PublicKey, limit: u64, rng: &mut R) -> Self
    where
        R: RngCore + CryptoRng,
    {
        ...
        RateLimiter {
            ...
            last_reset: RateLimiterMutex::new(Instant::now()),
        }
    }
    ...
    pub async fn reset_count(&self) {
        ...
        let mut last_reset_time = self.last_reset.lock().await;
    ...
    fn format_cookie_reply<'a>(
        ...
        if dst.len() < super::COOKIE_REPLY_SZ {
            warn!(
                "Cookie reply buffer too small: have={} need={}",
                dst.len(),
                super::COOKIE_REPLY_SZ
            );
    ...
    pub fn verify_packet<'a, 'b>(
        ...
        trace!("Verifying incoming WireGuard packet: len={}", src.len());
        ...
        | Packet::HandshakeResponse(HandshakeResponse { sender_idx, .. }) = packet
        {
            ...
            if !bool::from(computed_mac1[..16].ct_eq(mac1)) {
                debug!(
                    "Rejecting handshake packet: invalid mac1 sender_idx={}",
                    sender_idx
                );
            ...
            if self.is_under_load() {
                trace!(
                    "Rate limiter under load while verifying sender_idx={}",
                    sender_idx
                );
                let addr = match src_addr {
                    None => {
                        trace!("Rejecting handshake packet: under load and no source address");
                ...
                if !bool::from(computed_mac2[..16].ct_eq(mac2)) {
                    trace!(
                        "Handshake packet needs cookie reply: sender_idx={}",
                        sender_idx
                    );
